##run script many times to get a dataset of minimums
library(tidyverse)
library(bbmle)
library(mgcv)
library(stargazer)
...
mins1 = read.csv("output/results/minimums_de.csv")
mins2 = read.csv("output/results/small_minimums_de.csv")
...
plot100 = ggplot(data = mins1, aes(x = prop, y = dist.km)) +
...
plot10 = ggplot(data = mins2, aes(x = prop, y = dist.km)) +
  geom_boxplot(aes(group = prop), alpha = 0.5) +
...
##looking at all minimums across all proportions tested
newmins = mins %>% filter(prop != 100)
fit4 = lm(dist.km ~ prop, data = mins)
summary(fit4)
fit5 = lm(log(dist.km) ~ prop, data = mins)
plot(fit5, which = 2)
summary(fit5)
...
plot1 = ggplot(data = mins, aes(x = prop, y = dist.km)) +
  geom_boxplot(aes(group = prop), alpha = 0.5) +
...
# ggsave("output/figures/power-fit-abs-dist.png", eplot1, 
